...
服务端采用“默认值注入 + 显式参数优先”的策略，而不是全局强制 raw 或强制 streaming。

含义如下：

- 当客户端未显式传 `media_type` 时，服务端默认注入 `raw`
- 当客户端未显式传 `streaming_mode` 时，服务端默认注入 `3`
...
服务端显式引入 `session_id` 作为参考音频与 speaker 默认状态的会话键。
...
错误正文中应包含稳定关键字，避免客户端只能依赖模糊字符串匹配。
...
聊天窗启动时生成一个稳定的 TTS `session_id`，在该窗口生命周期内复用。

用途：

- 启动后异步调用 `/set_refer_audio` 预热该 `session_id`
- 逐句 `/tts` 请求复用同一 `session_id`
...
- 给 `/set_refer_audio` 与 `/tts` 请求都带上 `session_id`
- 统一拼装两套参数映射
- 识别可回退错误
...
- 服务端使用“默认值注入 + 显式参数优先”
- 服务端引入显式 `session_id` 做 speaker 会话化
- 客户端自动模式优先尝试 PCM，失败后在当前聊天窗会话内锁定为 WAV

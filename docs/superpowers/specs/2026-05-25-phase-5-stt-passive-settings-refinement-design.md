...
> 状态：设计已实施，Phase 5 核心改进基本完成；本轮按用户要求不执行自动化测试，后续由用户实机测试和真实服务联调验证。
...
- 重排系统设置外观区域，降低拥挤感。
- 字体选择使用系统字体下拉框。
- 字体下拉框的选项按字体自身样式预览；遇到 `Fixedsys` 等不可平滑缩放字体时只保留名称，不使用该字体渲染预览。
...
- 字体项预览只对 `QFontDatabase.isSmoothlyScalable()` 返回真的字体启用，避免 Windows / Qt 对不可缩放字体输出 DirectWrite 警告。
...
- 系统页字体控件使用系统字体下拉框。
- 系统页字体下拉框只预览可平滑缩放字体，保留不可预览字体名称。
- 被动气泡设置集中在“被动气泡”分组，不与聊天显示或被动状态阈值混放。
...
- 字体通过系统字体下拉框选择，可预览字体使用自身样式展示。
- 气泡缩放、最大宽度和停留时长集中放在“被动气泡”分组。
- API 页和系统页保存语义隔离。
- 系统页保存后，已打开聊天窗立即应用新外观和被动状态参数。

## 当前进度

- 已完成：faster-whisper 本地服务 STT 配置与适配器、OpenAI Whisper 当前路径移除、聊天窗运行态被动状态、右键菜单手动切换、系统页独立保存和保存后应用。
- 已完成：ASR 页面字段命名与 TTS 对齐为 `API URL`，系统页拆分为基础外观、聊天显示、被动状态、被动气泡和网络。
- 已完成：字体下拉框枚举系统字体，可预览字体按自身样式展示；不可平滑缩放字体保留名称但不启用预览渲染。
- 待完成：用户实机测试；真实麦克风、真实 faster-whisper 服务、真实 STT / TTS / API 互锁体验仍需本地联调。

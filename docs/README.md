...
Yumetsuki（梦月）是一个 Python 桌宠 AI 伴侣项目，第三阶段与第四阶段已完成，第五阶段基础链路已落地，当前进入 faster-whisper、被动状态和系统设置保存语义的改进实现准备。
...
- [Phase 5：桌宠体验、UI 与 STT 设计](./superpowers/specs/2026-05-24-phase-5-ui-stt-design.md)
  第五阶段的桌宠体验、界面与语音输入设计
- [Phase 5 改进：faster-whisper、本地被动状态与系统设置设计](./superpowers/specs/2026-05-25-phase-5-stt-passive-settings-refinement-design.md)
  第五阶段基础实现后的方向修正：本地 faster-whisper、运行态被动状态、字体下拉框与系统页独立保存
- [Phase 5 改进实施计划](./superpowers/plans/2026-05-25-phase-5-stt-passive-settings-refinement-implementation.md)
  已确认设计的 TDD 实施拆解
...
- 第三阶段：已完成
  ...
  - 日志覆盖补强：记忆检索、短期上下文构建、LLM 流式进度、LLM 本地切句、TTS 入队与翻译完成均已接入平台日志；`llm.stream_progress` 已节流
...
- 第五阶段：进行中，基础能力已落地，改进设计与计划已确认
  - 显示配置：`SystemConfig.chat_display` 已支持聊天字体倍率和气泡倍率，系统设置页可编辑，聊天窗启动时应用到字体、按钮、输入框、边框与气泡尺寸。
  - 被动互动气泡：基础气泡展示已落地；下一步将从系统设置开关改为聊天窗运行态，支持空闲阈值自动进入和右键菜单手动切换。
  - STT 配置：`APIConfig.asr` 已支持引擎、Base URL、API Key、模型、语言、录音超时、静音阈值和静音时长。
  - STT 适配层：`stt/` 已提供 `STTResult`、`STTAdapter`、`OpenAIWhisperAdapter` 和 `STTManager`，当前主路径面向 OpenAI Whisper 兼容转写接口。
  - STT 录音层：`ui/chat/stt_recorder.py` 已提供 Qt 麦克风录音、PCM 静音检测、超时停止和 WAV 字节生成。
  - STT 主链路：聊天窗麦克风按钮已接入录音、转写 worker 和 `_on_send()`，识别文本复用现有 Agent、SessionContext、日志与 TTS 管线；关闭窗口时治理 recorder 与 worker 生命周期。
  - 已确认改进：STT 将只保留 faster-whisper 本地服务接口并删除 `openai_whisper`；系统设置字体改为系统字体下拉框；系统页将拥有独立保存按钮，保存后应用到已打开聊天窗。
  - 当前边界：离线自动化测试已覆盖配置、气泡、录音、适配器和聊天窗 STT 主链路；真实麦克风、真实 faster-whisper 服务和真实 STT / TTS / API 互锁体验仍需本地联调。
- 第六阶段：已确认范围，暂未展开实施计划
  - 插件 / MCP 管理增强
  - 浏览器高级操控
  - OCR 与视觉能力
...
- Phase 5 先按改进计划替换 STT 本地服务适配、被动状态模型和系统页保存语义，再做真实麦克风、真实 faster-whisper 服务、真实 STT / TTS / API 场景联调
...
3. `docs/superpowers/specs/2026-05-25-phase-5-stt-passive-settings-refinement-design.md`
4. `docs/superpowers/plans/2026-05-25-phase-5-stt-passive-settings-refinement-implementation.md`

...
当前主 UI 为 Tauri + Vue3 + Pinia + TypeScript 前端和 Python `python_core` headless sidecar。历史 PySide6 入口、旧 `ui/` 主实现和 PySide6 运行时依赖已从主线退场，但产品级功能和 UI parity 尚未完成。
...
- 状态：Tauri/Vue 已是当前主 UI，但当前只应视为技术迁移骨架完成，不应视为旧版产品完整复刻完成。
...
- 当前门槛：测试映射、RPC contract、no-PySide6 sidecar、发布安全 / 性能 gate 和文档 stale gate 必须持续通过；同时需要按 parity 恢复计划补齐旧版功能和 UI 样式。
...
- [Tauri 产品 Parity 恢复计划](./tauri-parity-recovery.md)
  当前迁移失败复盘、旧版截图获取方式、功能和 UI 恢复路线。

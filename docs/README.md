...
Yumetsuki（梦月）是一个 Python 桌宠 AI 伴侣项目，第三阶段与第四阶段已完成，第五阶段核心链路和 faster-whisper / 被动状态 / 系统设置保存语义改进已完成，第六阶段实现、本地自动化验证和 sub agent 复审已完成；真实服务和本机设备验收暂缓。
...
- `CLAUDE.md`
  面向 AI / 协作者的最小工作上下文
- [并发 5 Agent 协作 Prompt 模板](./agent-prompts/parallel-workers.md)
  用于中转站并发受限时的主控、实现、测试、审查和集成 worker 调度
...
- 第六阶段：实现完成，本地自动化验证和 sub agent 复审通过，实机验收暂缓
  ...
  - 启动窗口、插件 / MCP 页面拆分、设置中心字体与滚轮治理、系统日志来源配色等 UI 改进已完成本地验证。

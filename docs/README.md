...
- [OCR 与视觉输入](./vision-ocr.md)
  Tesseract OCR、显式触发和 `SessionContext` 注入
...
- 第六阶段：核心实现已完成，等待真实浏览器 / OCR 环境联调
  - 插件 / MCP 管理增强：插件加载状态、MCP 连接诊断、工具来源、HTTP 请求超时和重试配置已接入。
  - 浏览器高级操控：`web_automation` 已支持 Playwright 持续浏览器会话、导航、等待、提取、点击、填写、状态和关闭。
  - OCR 与视觉能力：已新增 `vision/` 模块，支持 Tesseract OCR、显式读屏触发和 `SessionContext.visual_observations` 注入。

## 下一步方向

- Phase 6 下一步做真实 Playwright Edge 会话、真实 Tesseract OCR 和第三方 MCP 服务端联调
...
8. `docs/plugin-web-automation.md`
9. `docs/vision-ocr.md`
10. `docs/service-tts-compatibility.md`
11. `docs/development.md`
12. `docs/ui-guidelines.md`

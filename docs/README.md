...
当前实现仍以 PySide6 为主 UI。2026-05-28 起新增 Tauri UI 重构设计线，目标架构为 Tauri + Vue3 + Pinia + TypeScript 前端和 Python headless sidecar；设计已定稿，实施计划已确认并进入 Phase 0 计划冻结与基线。该目标尚未完成代码迁移，当前不能把目标架构描述为已落地。
...
- [Tauri UI 重构发布级设计草案](./superpowers/specs/2026-05-28-tauri-ui-migration-design.md)
  当前 UI 重构的唯一设计入口；记录从 PySide6 迁移到 Tauri（Vue3 组合式 + Pinia + TypeScript）和 Python headless sidecar 的架构契约、复审 gate、退场计划与发布安全边界。当前已定稿，尚未完成代码迁移
- [Tauri UI 重构实施计划](./superpowers/plans/2026-05-29-tauri-ui-migration-implementation-plan.md)
  当前 UI 重构的实施入口；Phase 0 用于冻结测试映射、替换状态和迁移前基线，后续 Phase 再创建 Tauri / Vue / Rust 骨架和逐页迁移实现
- [Tauri UI 结构复刻规则](./tauri-ui-parity.md)
  Tauri UI 迁移的结构 parity 硬规则；规定每个 Vue / Tauri 页面或组件设计和实现前必须先读取对应 `ui/` 旧实现，并完整保留入口、层级、分组、控件项、状态语义和功能链路
...
- [Phase 7 实机验收待补记录](./superpowers/smoke/2026-05-28-phase-7-smoke.md)
  记录 Phase 7 自动化验证结果和后续真实 API / TTS / STT / OCR / MCP / Playwright Edge smoke 待补项
- [Phase 8 / Phase 9 阶段设计草案](./superpowers/specs/2026-05-28-phase-8-9-design.md)
  记录 Phase 8 的设计边界与 Phase 9 的路线图级意图；整体仍是草案
...
- [Phase 8-A 记忆账本实机验收待补记录](./superpowers/smoke/2026-05-28-phase-8-memory-ledger-smoke.md)
  记录真实 Mem0 本地向量库写入、重复候选跳过、失败日志和平台日志筛选的待补验收项
...
- 状态：设计已定稿，实施计划已确认，Phase 0 计划冻结与基线进行中；尚未实施 Tauri / Vue 页面迁移。
- 当前主 UI：PySide6。
- 目标 UI：Tauri shell + Vue3 组合式 + Pinia + TypeScript。
- 目标后端边界：Python headless sidecar 保留 Agent、LLM、TTS、STT、OCR、插件、MCP、配置、日志和记忆。
- 设计入口：[Tauri UI 重构发布级设计草案](./superpowers/specs/2026-05-28-tauri-ui-migration-design.md)。
- 实施计划：[Tauri UI 重构实施计划](./superpowers/plans/2026-05-29-tauri-ui-migration-implementation-plan.md)。
- 硬门槛：复审每项 90+ 后才能进入实施；全部迁移完成后完全移除 PySide6。
- 结构复刻规则：[Tauri UI 结构复刻规则](./tauri-ui-parity.md)。迁移时 `ui/` 源码是硬性结构来源，截图只作为辅助参考。
...
- 第五阶段：已完成，进入稳定化维护
  ...
  - STT 配置：`APIConfig.asr` 已收敛为本地 faster-whisper 库参数：引擎、模型目录、设备、计算类型、识别超时、语言、录音超时、静音阈值、静音结束时长和起始静音等待；默认使用 `cpu/int8`，模型目录优先使用 `data/models/stt/`，并兼容旧版 `data/models/` 直属目录。
...
- 第六阶段：实现完成，本地自动化验证和 sub agent 复审通过，实机验收暂缓
  ...
  - 浏览器与 OCR 实机反馈收口：默认浏览器搜索词会归一化，当前页面阅读和默认浏览器链式点击 / 阅读会走 OCR guard，不会误开 Playwright 空白会话或把阅读意图再次搜索。
  - 被动状态定时读屏：系统页可显式开启并配置间隔，默认关闭；进入被动状态后按间隔采集 OCR 摘要，写入最近视觉观察并供主动发言参考；截图中间产物按保留时长、数量上限和清理间隔自动清理。
  - 日志与 UI 实机反馈收口：平台日志结构化列表更紧凑、可拉伸、不横向滚动并展示关键工具参数；对话日志正文与工具 / 记忆 meta 分区；日志拖选文本时自动刷新不会弹掉；设置中心 tooltip 统一为 Sakura 浅色主题。
...
- Phase 7：已完成，实机验收待补
  ...
  - 待补记录：`docs/superpowers/smoke/2026-05-28-phase-7-smoke.md`
- Phase 8 / Phase 9：整体已形成阶段设计草案，尚未整体拆成实施计划
...
- Phase 8-A：记忆质量治理与可回溯记忆账本已完成，本地自动化验证通过；真实 Mem0 本地向量库写入联调待补
  - 计划文档：`docs/superpowers/plans/2026-05-28-phase-8-memory-ledger.md`
  - 待补记录：`docs/superpowers/smoke/2026-05-28-phase-8-memory-ledger-smoke.md`

## 下一步方向

- Phase 7 实机验收恢复后，优先补记录并确认诊断包脱敏结果。
- Phase 8 下一步优先讨论是否进入会话状态画像与关系连续性；Phase 7 实机 smoke 和 Phase 8-A 真实 Mem0 联调仍作为 1.0 验收门和真实链路风险收口项。

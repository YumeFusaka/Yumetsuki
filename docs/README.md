...
当前实现仍以 PySide6 为主 UI。2026-05-28 起新增 Tauri UI 重构设计线，目标架构为 Tauri + Vue3 + Pinia + TypeScript 前端和 Python headless sidecar；该目标尚未进入代码实施，当前只处于设计冻结前复审与补强阶段。
...
- [Tauri UI 重构发布级设计草案](./superpowers/specs/2026-05-28-tauri-ui-migration-design.md)
  当前 UI 重构的唯一设计入口；记录从 PySide6 迁移到 Tauri（Vue3 组合式 + Pinia + TypeScript）和 Python headless sidecar 的架构契约、复审 gate、退场计划与发布安全边界。当前已定稿，尚未实施代码迁移
...
- 硬门槛：复审每项 90+ 后才能进入实施计划；全部迁移完成后完全移除 PySide6。

...
→ ChatWindow 按 passive_observation_interval_seconds 节流截图
→ 后台 PassiveVisionWorker 执行 OCR
→ AgentManager.record_visual_observation_result 写入 SessionContext.visual_observations
...
→ ChatWindow 按 screenshot_cleanup_interval_minutes 定时执行强制清理

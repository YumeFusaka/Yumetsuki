...
- 第三阶段：已完成
  ...
  - 句级增量 TTS 播报接入：`SettingsWindow` 透传 `api.tts`，`ChatWindow` 在流式出字时按 `。！？；` 与换行切句，后台调用 GPT-SoVITS 合成，并按句序用 Qt 多媒体播放；适配器会把基础地址规范化到 `/tts`，支持参考音频 / 参考语言 / 参考文本配置，并兼容 `jp -> ja` 等常见语言别名；TTS 设置页支持参考音频浏览按钮与主流语言预置下拉框；输出语言可独立配置，并在句级 TTS 管线中按需逐句翻译，强制保证播报语言与设置一致；单句失败只跳过当前句，不阻塞文本对话

...
当前实现仍以 PySide6 为主 UI。Tauri UI 重构设计已定稿，实施计划已确认并进入 Phase 0 计划冻结与基线；目标为 Tauri shell + Vue3 组合式 + Pinia + TypeScript 前端，Python 作为 headless sidecar。该目标尚未完成代码迁移，不能把目标架构描述为已落地。
...
- 结构复刻规则：`docs/tauri-ui-parity.md`
- 目标通信链路：Vue typed client -> Tauri invoke/events -> Python stdio RPC。
- 目标退场：全部 UI 迁移完成后完全移除 PySide6、旧 `ui/` 实现和旧文档入口。
- 门槛：复审每项 90+ 后才进入实施；每阶段合并前必须完成对应本地 gate。
- UI 迁移硬约束：每个 Vue / Tauri 页面或组件设计、实现前必须先读取对应 `ui/` 旧实现文件，并产出旧 UI 对照摘要；必须完整保留入口、层级、分组、控件项、菜单 / 弹窗、状态语义和功能链路，样式允许 Web 化但不得自由重构结构。
...
- 第五阶段：已完成，进入稳定化维护
  ...
  - 录音链路已落地：`ui/chat/stt_recorder.py` 使用 Qt 麦克风输入采集 PCM，支持起始静音等待、静音检测、超时停止和 WAV 字节生成。
...
- 第六阶段：实现完成，本地自动化验证和 sub agent 复审通过，实机验收暂缓
  ...
  - 默认浏览器搜索路由已修正：`搜索 xxx`、`帮我搜一下 xxx`、`用浏览器搜索 xxx`、`打开浏览器搜索 xxx` 等会提取真实搜索词；LLM 工具调用边界也会二次归一化，避免把整句意图作为搜索语料。
  - 默认浏览器与 Playwright 会话边界已加固：未明确要求自动化浏览器 / Playwright / 持续会话时，`web_session_open` 会被拦截；当前页面阅读和默认浏览器上下文下的“打开第二个条目 / 看看这个结果”会走 OCR guard，避免误开空白自动化浏览器或把阅读意图再次搜索。
  - OCR 与视觉输入已接入：新增 `vision/` 模块，默认使用 RapidOCR，支持 PaddleOCR 进阶可选后端、显式读屏触发和 `SessionContext.visual_observations` 注入。
  - 被动状态定时读屏已接入：系统页可显式开启并配置间隔，默认关闭；只在聊天窗被动状态下触发，OCR 摘要会写入最近视觉观察并供主动发言参考；读屏截图支持按保留时长、数量上限和清理间隔定期清理。
  - 启动窗口、插件 / MCP 页面拆分、设置中心字体与滚轮治理、平台日志来源配色等第六阶段 UI 改进已完成本地验证。
  - 产品级收口已补充：
    - 聊天窗新增运行状态条，覆盖思考、读屏、回复、语音合成、失败重试和打开平台日志；发送按钮在忙碌态承担停止当前生成语义
    - 日志工作台新增实机反馈收口：平台日志更紧凑且上下可拉伸，结构化摘要展示 planner/tool 关键参数；对话日志正文与工具 / 记忆 meta 分区；拖选文本时自动刷新跳过；tooltip 统一浅色主题
    - 流式回复显示按短间隔合帧刷新，减少每个 chunk 触发完整富文本重绘；TTS 句段提交仍按原链路即时推进
    - 立绘管理器缓存原图和近期缩放结果，降低滚轮缩放和情绪切换时重复读图 / 平滑缩放成本
    - TTS 音频回传性能已补强：事件队列改为 O(1) 头部消费，GPT-SoVITS PCM / WAV 回包均采用有界 chunk，PCM 缓冲回收已读数据，WAV 句段改用临时文件播放并在播放、取消或关闭时清理
    - 启动加载窗已支持无边框拖拽，主体、标题、状态文字和进度条区域均可拖动窗口
  - 运行期中间产物目录 `data/browser_sessions/` 与 `data/vision/` 默认不提交。

## 下一步

- Phase 7 已完成；真实 API / TTS / STT / OCR / MCP / Playwright Edge 实机验收待补，恢复后按 smoke 记录模板补齐结果。
- Phase 8 / Phase 9 整体仍是阶段设计草案；Phase 8-A 作为第一项记忆治理切片已完成，其余 Phase 8 工作流需等 Phase 7 实机 smoke 和 Phase 8-A 真实 Mem0 联调风险收口后再拆。
...
- 1.0 完成状态以 `docs/development.md` 中的 1.0 验收门为准；未完成项需分类为 `post-1.0`、`maintenance` 或 `won't do`。
- 暂缓实机验收，后续恢复时优先检查：
...
- [文档入口](./docs/README.md)
- [Tauri UI 重构发布级设计草案](./docs/superpowers/specs/2026-05-28-tauri-ui-migration-design.md)
- [Tauri UI 重构实施计划](./docs/superpowers/plans/2026-05-29-tauri-ui-migration-implementation-plan.md)
- [Tauri UI 结构复刻规则](./docs/tauri-ui-parity.md)
...
- [后续阶段功能头脑风暴记录（历史参考）](./docs/superpowers/brainstorms/2026-05-28-future-stage-brainstorm.md)
- [Phase 7 实机验收待补记录](./docs/superpowers/smoke/2026-05-28-phase-7-smoke.md)
- [Phase 8 / Phase 9 阶段设计草案](./docs/superpowers/specs/2026-05-28-phase-8-9-design.md)
- [Phase 8-A 记忆质量治理实施计划](./docs/superpowers/plans/2026-05-28-phase-8-memory-ledger.md)
- [Phase 8-A 记忆账本实机验收待补记录](./docs/superpowers/smoke/2026-05-28-phase-8-memory-ledger-smoke.md)

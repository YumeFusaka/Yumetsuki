...
- 每次只输出一步
- 优先选择能一步完成用户意图的复合工具，不要把它拆成多步。例如：用户说"打开浏览器搜索 xxx"，应直接选 web_automation__web_search_visible（一次完成"打开可见浏览器+搜索"），不要先 open_browser 再 web_search
- 仔细阅读工具 description，识别"可见/后台"、"打开窗口/静默"等关键差异，根据用户原意选择匹配的那一个"""
